        ...
        do {
            ...
            let detectionRequest = VNCoreMLRequest(model: detectionModel!) { [weak self] request, error in
                ...
                DispatchQueue.main.async {
                    ...
                    if isCentered && !self.isAvocado && !detected.isEmpty {
                        self.isAvocado = true
//                        self.capturePhoto()
                    }else{
        ...
        guard let classificationMLModel = try? Avocado_v1_60(configuration: MLModelConfiguration()) else {
